...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = (await listTrackedAccountBundles()).find(bundle => bundle.account.id === id);
  ...
  try {
    ...
    const items = await persistMarketItems(await provider.getAccountWorks(input));
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  await deleteTrackedAccountBundle(id);
