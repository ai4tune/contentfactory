...
test("Xiaohongshu keyword links are encoded and search sessions are portable", () => {
  ...
  assert.equal(JSON.parse(archive.json).schema, "contentfactory.search-session.v1");
});

test("visible Xiaohongshu cards keep title, author, likes, date, and canonical URL aligned", () => {
  ...
  const nodes = {
    ...
    time: { textContent: "2天前" },
    image: { currentSrc: "https://ci.xiaohongshu.com/demo.jpg", src: "" },
  };
  const root = {
    textContent: "企业 AI 落地复盘 杏仁聊AI 2天前 1.8万",
